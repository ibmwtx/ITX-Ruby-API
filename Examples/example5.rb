...
rc = Map.mpiMapLoadFile(hMap, "test5.mmc")
check_err(rc)
...
szInputBuffer = "This is my input data"
rc = Stream.mpiStreamWrite(hStreamPtr, szInputBuffer, szInputBuffer.length)
check_err(rc)

# Get the adapter object handle for output card #2
rc = Map.mpiMapGetOutputCardObject(hMapPtr, 2, hCard)
check_err(rc)
